    ...
    asyncLib.waterfall([
        ...
        models.Project.findOne({
          ...
          .catch((err) => {
            return res.status(500).json({
              'error': 'impossible de trouver le projet'
            });
          });
      },
      (projectFound, done) => {
        if (projectFound) {
          ...
          res.status(404).json({
            'error': 'impossible de liker, vous devez vous connecter'
          });
        }
      },
      (projectFound, userFound, done) => {
        if (userFound) {
          models.ProjectsLikes.findOne({
            ...
            .catch((err) => {
              return res.status(500).json({
                'error': 'impossible de vérifier si l\'utilisateur a déjà liké' + err
              });
            });
        } else {
          res.status(404).json({
      ...
      (projectFound, userFound, userAlreadyLikedFound, done) => {
        if (!userAlreadyLikedFound) {
          projectFound.addUser(userFound)
            ...
            .catch((err) => {
              return res.status(500).json({
                'error': 'impossible de voter pour ce projet' + err
              });
            });
        } else {
          if (userAlreadyLikedFound.isLike === DISLIKED) {
            ...
            res.status(409).json({
              'error': ' Vous avez déjà liké ce projet'
            });
          }
        }
      },
      (projectFound, userFound, done) => {
        projectFound.update({
          ...
          .catch((err) => {
            res.status(500).json({
              'error': 'impossible de mettre à jour le compteur de likes' + err
            });
          });
      },
    ], (projectFound) => {
      if (projectFound) {
        ...
        return res.status(500).json({
          'error': 'Impossible de mettre à jour le compteur'
        });
      }
    });
  },
  ...
  unlikePost: (req, res) => {
    ...
    asyncLib.waterfall([
      ...
      (projectFound, userFound, userAlreadyLikedFound, done) => {
        if (!userAlreadyLikedFound) {
          return res.status(500).json({
            'error': 'Vous ne pouvez pas disliker un projet que vous n\'avez pas liké'
          });
        } else {
          if (userAlreadyLikedFound.isLike === LIKED) {
            ...
            res.status(409).json({
              'error': ' Vous avez déjà disliké ce projet'
            });
          }
        }
      },
    ...
    models.ProjectsLikes.findAll({
      ...
      include: [{
        ...
        include: [{
          ...
          model: models.User,
          attributes: {
            exclude: ['createdAt', 'updatedAt', 'password', 'bio', 'isAdmin', 'email']
          }
        }]
      }, ],
    }).then((projectLikes) => {
