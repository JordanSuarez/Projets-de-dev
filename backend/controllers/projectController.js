  ...
  new: (req, res) => {
    ...
    if (description == null || title == null || image == null || tagId == null) {
      return res.status(500).json({
        'error': 'Titre, description, image et au moins 1 tag requis'
      });
    }
      ...
      (done) => {
        const newProject = models.Project.create({
          ...
          .catch((err) => {
            console.log
            return res.status(500).json({
              'error': 'Erreur lors de l\'ajout du nouveau projet: ' + err
            });
          });
      }
    ])
  ...
  deleteProject: (req, res) => {
    ...
    if (userId > 0 && isAdmin == true) {
      ...
      return res.status(401).json({
        'error': 'vous n\'avez pas l\'autorisation de supprimer ce projet'
      });
    }
  },
}